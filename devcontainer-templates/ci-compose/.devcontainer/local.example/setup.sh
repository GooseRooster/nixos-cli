...
# Installs Nix (kept out of the CI image), the nix-cli CLI bundle, and your
...
# 1) Nix — not baked into the CI image; personalization brings its own.
#    Multi-user install needs passwordless sudo; the installer's curl+git/gcc
#    prereqs assume a Debian/Ubuntu CI base (adjust for dnf/apk if yours differs).
...
#    hosts/devcontainer.nix read the real $USER/$HOME (uid-1000 user varies).
nix --extra-experimental-features 'nix-command flakes' \
