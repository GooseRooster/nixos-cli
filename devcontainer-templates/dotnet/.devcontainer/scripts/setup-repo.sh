...
# ── ⟨blazor⟩ Blazor WebAssembly extras (uncomment as needed) ──
# sass (SCSS build) — needs the node Feature enabled in devcontainer.json:
#   npm install -g sass
# NSwag CLI (regenerate API clients; scripts resolve `nswag` on PATH):
